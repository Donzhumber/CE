...
/-!
# Appendix B Proofs — Lean 4 Formalization

Standalone formal companion to `Appendix_B_Proofs.tex` for
*Identifying Rational Types in Unknown Environments under an Indirect Dynamic Mechanism* (Computational Economics supplement).

This file mirrors the **seven sections** of the TeX appendix. Each block is
...
fixed-point existence). Sections 3 & 7 algebra is machine-checked through
...
Bayesian §1 of `thm:posterior-consistency` is factored into
...
`coxTvSeparationLocal_of_betaSeparation`) are proved in §2 for `def:desenlace-fisico-mt`;
`asymptoticIdentification_from_coxBetaSeparation` wires them to §3 via `IdentificationBridge`.
Abstract `coxTvSeparationLocal` remains the general interface. Section~4 certifies
`kappa_h_signed`, `dLambda_eq_kappa`, `dContProb_eq`, and the sign of
`eq:dE-dgamma-prop` with explicit `Δt` and `survivalTail`. Section~5 records
...
| § | TeX section | `\\label` |
|---|-------------|-----------|
| 1 | Public information, likelihood, and Bayesian learning | `sec:info-bayes` |
| 2 | Stochastic implementation mechanism (MDG) | `sec:mdg` |
| 3 | Asymptotic type identification | `sec:id-asintotica` |
| 4 | Competitive risks and operational pressure | `sec:riesgos` |
| 5 | Policy implications | `sec:politica` |
| 6 | Equilibrium and implementability | `sec:equilibrio` |
| 7 | Abstract structural identification (Kakutani criterion) | `sec:id-abstracta` |
-/
...
/-!
## Section 1 — Public information, likelihood, and Bayesian learning (`sec:info-bayes`)
...
/-!
## Section 2 — Stochastic implementation mechanism (MDG) (`sec:mdg`)
...
/-!
## Section 3 — Asymptotic type identification (`sec:id-asintotica`)
...
/-!
## Section 4 — Competitive risks and operational pressure (`sec:riesgos`)
...
/-!
## Section 5 — Policy implications (`sec:politica`)
...
/-!
## Section 6 — Equilibrium and implementability (`sec:equilibrio`)
...
/-!
## Section 7 — Abstract structural identification (`sec:id-abstracta`)
